...
def get_populartimes_from_search(place_identifier):
    ...
    popular_times = format_popular_times(popular_times)
...
def format_popular_times(details):
    # Today's date
    date = datetime.datetime.now()

    weekDay = date.weekday() + 1
    details = details[weekDay][1]
    result = []
    
    for i in range(len(details)):
        result.append({
            "time": details[i][0], 
            "time_string": details[i][4], 
            "busy_score": details[i][1]
        })
...
def run(_params):
    ...
    return results
#get_populartimes("AIzaSyD6JCMB88SJ1InPCIrrw81WsFZpzpa8n78", "ChIJ5Y6JgMY1joAR3lBYIiAnJoo")

#start process
if __name__ == '__main__':
    # print(str(sys.argv[1]))
    get_populartimes_from_search(sys.argv[1])
    # get_populartimes_from_search("Whole Foods Market 2732 Augustine Drive Ste 1600, Santa Clara")
    # get_populartimes_from_search("SF Party 939 Post St, San Francisco, CA 94109, USA")
    # get_populartimes_from_search("Antique Vibrator Museum 1620 Polk St, San Francisco, CA 94109, USA")
    # get_populartimes_from_search("Upcider 1160 Polk St 2nd floor, San Francisco, CA 94109")
    # get_populartimes('AIzaSyD6JCMB88SJ1InPCIrrw81WsFZpzpa8n78', 'ChIJR20MfcY1joARQSEVvaP0vsM')
